...
const OpenAIForm: FC<OpenAIFormProps> = ({
  state,
  prevNode,
  execute,
  runData,
  ...
  const [formValues, setFormValues] = useState({
    ...
    model: initialValues?.model || "gpt-4o-mini",
  });
  ...
  useEffect(() => {
    const getCreds = async () => {
      const creds = await getCredential("OpenAINode");
        ...
        <div className="max-w-[400px] w-full flex flex-col overflow-y-auto h-full shadow-sm rounded-xl border p-4">
          <div className="pb-4 border-b mb-8 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-extrabold">OpenAI Chat Model</h2>
            </div>
            {state !== "running" && (
